...
#          embeddedDict['list']=setsOfReps
  ...
  #len of each list in dictionary[key] will be a representation of sets and reps
    ...
    if timesPerWeek>0:
      ...
      possibleFrequencies=[]
      for iterator in possibleFrequenciesOriginal:
        possibleFrequencies.append(iterator)
      if muscleGroupObject.frequencyException.count()>0:
        for muscleFrequencyObject in muscleGroupObject.frequencyException.all():
          #toReplace=MuscleFrequency.objects.filter(name=muscleFrequencyObject.name, exception=False)[0]
          toReplace=toReplaceDict[muscleFrequencyObject.name.__str__()]
          index=possibleFrequencies.index(toReplace)
          possibleFrequencies[index]=muscleFrequencyObject
        #all you need to do is replace the exception you'd normally use with the one with the same name
#mid and mid high get pissed away
      for frequency in possibleFrequencies:
        if frequency.minReps>averageReps or frequency.maxReps<averageReps or frequency.maxSets<averageSets:
          toRemove.append(frequency)
      for remove in toRemove:
        possibleFrequencies.remove(remove)
...
##############################
  ...
  componentDict={}
  allComponentFrequencies=Frequency.objects.filter(currentFitnessLevel=userInformation.currentFitnessLevel, week=currentWeekToUse).select_related('workoutComponent')
...
#      if not exists:
        ...
        thisWeek=ThisWeek(dayNumber=j, user=user, cardio=True, timed=True, level=1)#put cardio on a day automatically if there isn't anything
...
#    smallest=myDays[0].workoutComponents.count()
    smallest=myDaysCount[0]
    toRemove=myDays[0]
    for iterator in myDays:
      if myDaysCount[myDays.index(iterator)]<smallest:
#      if iterator.workoutComponents.count()<smallest:
#        smallest=iterator.workoutComponents.count()
        smallest=myDaysCount[myDays.index(iterator)]
        toRemove=iterator
    index=myDays.index(toRemove)
#    myDaysCount.pop([index])
  ...
  for j in range(0, timedCardioDays):
    try:
      ...
      pass
#      from django.core.mail import send_mail
#      send_mail("OracleFitness Stat DataERROR", "ERROR", 'administrator@oraclefitness.com', ['scott.lobdell@gmail.com'], fail_silently=False)
  if False:#removed this crap
    for j in range(0, distanceCardioDays):
      sortedDays[j].cardio=True
      sortedDays[j].timed=False
      sortedDays[j].save()
...
#####################################################
  ...
  somePhase=userInformation.goal.phase.all()[userInformation.currentPhase]



  myCardioZone=CardioZone.objects.filter(fitnessLevel=userInformation.currentFitnessLevel, cardioType=userInformation.goal.cardioType)
  myPhase=somePhase
  ...
  for iterator in thisWeek:
    if iterator.workoutComponents.count()==0:
      toRemove.append(iterator)
  for iterator in toRemove:
    thisWeek.remove(iterator)
  ...
  for iterator in thisWeek:
    if iterator.cardio:
      cardioDays.append(iterator)
    else:
      nonCardioDays.append(iterator)
  ...
  for iterator in nonCardioDays:
    if not iterator.cardio and not (resistanceComponent in iterator.workoutComponents.all()) and iterator.workoutComponents.count()>0: #we have some standalone things...swap it out with a resistance on a cardio day
      deleted=False
  ...
  if len(nonCardioDays)>0:
    maxWeekInPhase=6
    currentWeekToUse=currentWeek
    if somePhase in userInformation.visitedPhases.all():
      currentWeekToUse=maxWeekInPhase

    volumeTable=VolumeTable.objects.filter(fitnessLevel=userInformation.currentFitnessLevel, phase=somePhase, week=currentWeekToUse)[0]
    dictionary={}
    allWorkoutComponents=WorkoutComponent.objects.all()
    for iterator in allWorkoutComponents:
      dictionary[iterator.name]=Volume.objects.filter(workoutComponent=iterator, parentTable=volumeTable)[0]


    def computeComponentWeight(WorkoutComponentObject):
        if WorkoutComponentObject.name=='Flexibility':
          return 0

        currentVolume=dictionary[WorkoutComponentObject.name]
        averageSets=float(currentVolume.minSets+currentVolume.maxSets)/2.0
        averageExercises=float(currentVolume.minExercises+currentVolume.maxExercises)/2.0
        averageReps=float(currentVolume.minReps+currentVolume.maxReps)/2.0
        val1=int(somePhase.tempo[0:1])
        val2=int(somePhase.tempo[2:3])
        val3=int(somePhase.tempo[4:5])
        if val1==0:
          val1=1
        if val2==0:
          val2=1
        if val3==0:
          val3=1
        oneRepTime=float(val1+val2+val3)
        restTime=0
        if WorkoutComponentObject.name=='Resistance':
          restTime=somePhase.rest
        else:
          restTime=45
#        writeTestData(((oneRepTime*averageReps*averageSets*averageExercises+averageSets*float(restTime))/60.0).__str__())
        return oneRepTime*averageReps*averageSets*averageExercises+averageSets*float(restTime)

    def computeTimeWeight(ThisWeekObject):
      timeWeight=0
      for WorkoutComponentObject in ThisWeekObject.workoutComponents.all():
        timeWeight=timeWeight+computeComponentWeight(WorkoutComponentObject)
      return timeWeight
#    writeTestData("Resistance Time for day "+ThisWeekObject.dayNumber.__str__()+": "+timeWeight.__str__())


    removeResistance=True
    maxOverallTime=0
    avgCount=0
    for iterator in cardioDays:
      newCardioZone=myCardioZone.filter(level=iterator.level)

      for CardioZoneObject in newCardioZone:
        if CardioZoneObject.maxOverall==0:
          pass
        else:
          avgCount=avgCount+1
          maxOverallTime=maxOverallTime+CardioZoneObject.maxOverall
#        if CardioZoneObject.maxOverall>maxOverallTime:
#          maxOverallTime=CardioZoneObject.maxOverall
      maxOverallTime=maxOverallTime/avgCount
#      writeTestData("Maxoverall is "+maxOverallTime.__str__())
    #maxOverallTime is now the longest amount of time cardio can be...now remove some other workout components
      greatestResistanceTime=0.0
      for iterator2 in nonCardioDays:
        resistanceTime=computeTimeWeight(iterator2)
        if resistanceTime>greatestResistanceTime:
          greatestResistanceTime=resistanceTime

      timeRemoved=0
      initialResistanceTime=computeTimeWeight(iterator)
      if False and not removeResistance and computeTimeWeight(iterator)+float(maxOverallTime)*60.0>greatestResistanceTime:
        removeResistance=True
#        writeTestData("YESSSS dropped a cardio!")
        iterator.cardio=False
        iterator.save()
        nonCardioDays.append(iterator)
      elif False:
        timeWasted=0.0
        while removeResistance and computeTimeWeight(iterator)+float(maxOverallTime)*60.0>(greatestResistanceTime*1.25) and iterator.workoutComponents.count()>0:
          #now simply move crap around
          randomWorkoutComponent=list(iterator.workoutComponents.all())
          #randomWorkoutComponent=randomWorkoutComponent[random.randint(0,len(randomWorkoutComponent)-1)]
          sortedComponents=[]
          while randomWorkoutComponent:
            smallest=randomWorkoutComponent[0]
            smallestTime=computeComponentWeight(smallest)
            for workoutComponentObject in randomWorkoutComponent:
              newTime=computeComponentWeight(workoutComponentObject)
              if newTime< smallestTime:
                smallestTime=newTime
                smallest=workoutComponentObject
            sortedComponents.append(smallest)

            randomWorkoutComponent.remove(workoutComponentObject)
          randomWorkoutComponent=sortedComponents
          randomWorkoutComponent=randomWorkoutComponent[0]
          iterator.workoutComponents.remove(randomWorkoutComponent)
#          writeTestData("removing "+randomWorkoutComponent.name)
          for j in range(0, len(nonCardioDays)):
            minimum=nonCardioDays[j].workoutComponents.count()
            for i in range(j, len(nonCardioDays)-1):
              if nonCardioDays[i].workoutComponents.count() < minimum:
                minimum=nonCardioDays[i].workoutComponents.count()
              temp=nonCardioDays[i]
              nonCardioDays[i]=nonCardioDays[j]
              nonCardioDays[j]=temp
          found=False
          for iterator2 in nonCardioDays:
            if (not randomWorkoutComponent in iterator2.workoutComponents.all()) and not found:
              found=True
              iterator2.workoutComponents.add(randomWorkoutComponent)
#              writeTestData("adding "+randomWorkoutComponent.name+ " elsewhere")
              #iterator2.save()
              resistanceTime=computeTimeWeight(iterator2)
              if resistanceTime>greatestResistanceTime:
                greatestResistanceTime=resistanceTime
          if not found:
            timeWasted=timeWasted+computeComponentWeight(randomWorkoutComponent)
#            writeTestData("dropping "+randomWorkoutComponent.name)
          if timeWasted>=float(maxOverallTime)*60.0:
            removeResistance=False
          iterator.save()
...
#PROBLEM, this thing won't work backwards
...
#        resistanceTime=oneRepTime*averageReps*averageSets*averageExercises+averageSets*float(somePhase.rest)
#        timeWeight=timeWeight+resistanceTime
#      return timeWeight
#STILL NEED USERINFO.goal and all that crap
...
#Above list does not include bodyweight exercises
...
#  for iterator in musclesToRemoveQuery:
#    musclesToRemove.append(iterator.muscleGroup.id)
...
#  for iterator in baseExercises:
#    myString=myString+iterator.name+", "
#  send_mail("potentials2", myString, 'administrator@oraclefitness.com', ['scott.lobdell@gmail.com'], fail_silently=False)
#Exercise.objects.filter(minFitnessLevel.value__gte=userInfo.currentFitnessLevel.value, minExerpience.value__gteuserInfo.experience.value)
  ...
  potentialExercisesBodyWeight=list(potentialExercisesBodyWeight.select_related('minFitnesslevel','minExperience','muscleGroup','secondaryMuscleGroup','workoutComponent','exerciseType2','workoutComponent2','workoutComponent3'))
    #potentialExercisesBodyWeight=list(Exercise.objects.filter(Q(workoutComponent__in=componentList) | Q(workoutComponent2__in=componentList) | Q(workoutComponent3__in=componentList), phase__in=[myPhase]).exclude( muscleGroup__in=yesterdayMuscles).exclude(muscleGroup__in=userInfo.avoidedMuscleGroup.all()).select_related('minFitnesslevel','minExperience','muscleGroup','secondaryMuscleGroup','workoutComponent','exerciseType2','workoutComponent2','workoutComponent3'))
...
#  for iterator in baseExercises:
#    myString=myString+iterator.name+", "
#  send_mail("potentials3", myString, 'administrator@oraclefitness.com', ['scott.lobdell@gmail.com'], fail_silently=False)
#    toRemove=[]
#    for iterator in potentialExercises:
#      if iterator.muscleGroup in yesterdayMuscles or iterator.muscleGroup in musclesToRemove:
#        toRemove.append(iterator)
#    for iterator in toRemove:
#      potentialExercises.remove(iterator)
  t2=datetime.datetime.now()
